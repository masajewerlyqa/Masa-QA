...
const GOLD_KARATS = [
  { value: '24', label: '24K', purity: 0.999, buybackRate: 0.98 },
  { value: '22', label: '22K', purity: 0.917, buybackRate: 0.95 },
  { value: '21', label: '21K', purity: 0.875, buybackRate: 0.93 },
  { value: '18', label: '18K', purity: 0.75, buybackRate: 0.9 },
  { value: '14', label: '14K', purity: 0.583, buybackRate: 0.85 },
];
...
export function SellGoldCalculatorScreen(): React.JSX.Element {
  ...
  const calculate = (): void => {
    ...
    const marketValue = weightNum * priceNum;
    ...
    <View style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <ToolPageHero
          ...
          subtitle={
            isArabic
              ? 'قدّر القيمة السوقية وعرض الشراء قبل البيع'
              : 'Estimate market value and buyback offer before selling'
          }
          title={t('tools.sell.title')}
        />
        <ContentContainer>
          <MasaCard style={styles.card}>
            ...
            <TextInput
              ...
              value={goldPrice}
            />
            <MasaButton label={isArabic ? 'قدّر القيمة' : 'Estimate Value'} onPress={calculate} />
          </MasaCard>
          {result ? (
            <MasaCard style={styles.card}>
              <Text style={[styles.resultTitle, { fontFamily: fontFamily(isArabic, 'luxury') }]}>
                {isArabic ? 'التقدير' : 'Estimate'}
              </Text>
              ...
              <Text style={styles.resultLine}>
                ...
                {formatQar(result.marketValue, isArabic)}
              </Text>
              <Text style={styles.resultHighlight}>
                {isArabic ? 'عرض تقديري:' : 'Estimated offer:'}{' '}
                {formatQar(result.estimatedOffer, isArabic)}
              </Text>
              <Text style={styles.resultNote}>
                {isArabic
                  ? `معدل إعادة الشراء: ${result.buybackRate}%`
                  : `Buyback rate: ${result.buybackRate}%`}
              </Text>
            </MasaCard>
          ) : null}
        </ContentContainer>
